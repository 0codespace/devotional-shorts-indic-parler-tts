...
# ============================================================================
# DEACTIVATED 2026-08-28 — DO NOT USE.
# This is the LEGACY path: edge-tts (hi-IN-SwaraNeural) + single-image Ken Burns.
# It was still running in production because ~/.ssh/authorized_keys pinned the
# n8n key to a forced command pointing here, which silently overrode the
# workflow's own command. That forced command now points at
# publish_short_main.sh (Indic Parler-TTS, Rani/Divya + crossfade slideshow).
# Production entry point is: publish_short_main.sh
# ============================================================================
echo "REFUSED: publish_short.sh is deactivated. Use publish_short_main.sh (Indic Parler-TTS)." >&2
exit 1

# Full pipeline: pick topic -> generate script -> render video -> upload to YouTube -> notify Telegram.
...

{
  ...
  # Without this, get_image.py never sees image_override and every video silently
  # falls back to the single shared category image.
...
PY
  ...
  bash render_video.sh "$SCRIPT_JSON" "$CATEGORY" "$OUTPUT_MP4"
